 ...
 * java.io.InputStream; import java.net.URI; import java.util.Properties;
 * 
 ...
 * 
 * public class MjpegInputStream extends DataInputStream { private final byte[]
 * SOI_MARKER = { (byte) 0xFF, (byte) 0xD8 }; private final byte[] EOF_MARKER =
 * { (byte) 0xFF, (byte) 0xD9 }; private final String CONTENT_LENGTH =
 * "Content-Length"; private final static int HEADER_MAX_LENGTH = 100; private
 * final static int FRAME_MAX_LENGTH = 40000 + HEADER_MAX_LENGTH; private int
 * mContentLength = -1;
 * 
 * public static MjpegInputStream read(String url) { HttpResponse res;
 * DefaultHttpClient httpclient = new DefaultHttpClient(); try { res =
 * httpclient.execute(new HttpGet(URI.create(url))); return new
 * MjpegInputStream(res.getEntity().getContent()); } catch
 * (ClientProtocolException e) { } catch (IOException e) {} return null; }
 * 
 ...
 * seqIndex++; if(seqIndex == sequence.length) return i + 1; } else seqIndex =
 * 0; } return -1; }
 * 
